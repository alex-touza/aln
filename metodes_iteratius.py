...
class MetodeIteratiu(ABC):
    ...
    Retorna dues matrius M, N corresponents a la descomposició
    regular associada al mètode iteratiu.
    """
    @abstractmethod
    def descompondre(self, A):
        pass

    @final
    def resoldre(self, A, b, x):
        nit = 0
        r = b - A @ x
...
class SobreRelaxacioSuccessiva(MetodeIteratiu):
    def __init__(self, tol: np.float64, nitm: int, omega: np.float64 = np.float64(1)):
        assert 0 <= omega <= 2
    ...
    def trobar_omega_optima(self, A, b, x):
        ...
        for i in range(len(omega)):
            ...
            x,r,it = sor(A,b,x,omega[i],self.tol,self.nitm)
            print(f"i: {i:3d}, " +
